<div>
    <div class="search-bar mb-4 flex flex-col sm:flex-row items-stretch sm:items-center">
        ...
        <button wire:click="searchJobs" class="bg-blue-600 text-white p-2 rounded mb-2 sm:mb-0 sm:mr-2">Search</button>
        <button wire:click="resetFilters" class="bg-red-600 text-white p-2 rounded">Reset All</button>
    </div>
                ...
                <div class="flex-1 text-center lg:text-left">
                    ...
                    <div class="flex flex-col lg:flex-row items-center lg:items-center text-gray-600 mt-2">
                        <div class="flex items-center mr-4 mb-2 lg:mb-0">
                            <span><i class="bi bi-geo-alt-fill text-yellow-600"></i><span class="font-bold ml-1">Location:</span> {{ $job->location }}</span>
                        </div>
                        <div class="flex items-center mr-4 mb-2 lg:mb-0">
                            <span><i class="bi bi-cash-coin text-yellow-600"></i><span class="font-bold ml-1">Salary:</span> {{ $job->salary }}</span>
                        </div>
                        <div class="flex items-center mr-4 mb-2 lg:mb-0">
                            <span><i class="bi bi-calendar-event text-yellow-600"></i><span class="font-bold ml-1">Posted on:</span> {{ $job->created_at->format('d M Y') }}</span>
                        </div>
                    </div>
                </div>
                <a href="{{ route('jobs.details', $job->id) }}" class="bg-yellow-600 text-white p-2 rounded mt-4 lg:mt-0 ml-0 lg:ml-6">View job details</a>
            </div>
        @endforeach
